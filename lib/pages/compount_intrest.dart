...
class _CompoundinterestState extends State<Compoundinterest> {
  ...
  void _calculateInterest() {
    setState(() {
      ...
      interest = principal * pow((1 + rate / 100), term) - principal;
    });
  }

  void _saveCurrentValues() {
    if (rate == 0.0 && term == 0.0) {
      Fluttertoast.showToast(
        msg: "Please enter any valid values to save",
    ...
    setState(() {
      ...
      final compoundInterestBox =
          Hive.box<CompountInterestModel>('compoundInterestBox');
      final newRecord = CompountInterestModel(
        cinterest: interest,
        ctotalAmount: principal + interest,
        ctermLength: selectedUnit,
        ctermValue: termController.text,
        ctitlename: "Compount Interest",
        ctimestamp: timestampStr,
      );

      compoundInterestBox.add(newRecord);
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final termValue = termUnit.isNotEmpty
                      ? termUnit[reversedIndex]['termvalue'] ?? ''
                      : '';
                  return SafeArea(
                    child: Card(
                      shape: RoundedRectangleBorder(
